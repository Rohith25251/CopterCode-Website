    ...
    useEffect(() => {
        ...
        const fetchFromSupabase = async (code) => {
            ...
            const { data, error: dbError } = await supabase
                .from('certificates')
                .select('cert_code, name, college, batch, year, department, role, issue_date, expiry_date, status, pdf_url, created_at, revoked_at, revoke_reason')
                ...
                {!loading && certificate && (
                    <div className="bg-white border border-border rounded-2xl p-8 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            {certificate.year && <div><span className="text-secondary">Year of Study</span><p className="font-semibold">{certificate.year}</p></div>}
                            {certificate.batch && <div><span className="text-secondary">Batch</span><p className="font-semibold">{certificate.batch}</p></div>}
